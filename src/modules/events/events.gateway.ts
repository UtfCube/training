...
export class EventsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async registerUser(client: Client, data: any): Promise<any> {
    ...
    const fr = frames[client.id].map(f => f.nose);
    const max = Math.max(...fr);
    const index_max = fr.indexOf(max);
    const left = index_max - 2;
    const right = index_max + 3;
    const gen_data = fr.slice(left, right);
    const target_func = create_target_func(gen_data);

    let genetic = new GeneticEvolution(target_func);

    genetic.initialize()
    let coefs = genetic.evolute();
    if (coefs.b > 0.25 && coefs.b < 0.47 && coefs.a < -0.063 && coefs.a > -0.11) {
